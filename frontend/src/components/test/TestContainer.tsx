...
const TestContainer = ({
  ...
  return (
    <div ref={containerRef} tabIndex={-1} className="flex flex-col outline-none">
      ...
      {viewMode === "student" && (
        <h1 className="text-2xl font-bold text-white mb-8">{testData.title}</h1>
      )}

      {/* Navigation */}
      <div className="flex flex-wrap items-center justify-between gap-4 my-4 mb-10 overflow-visible">
        <TestNavigation
          totalQuestions={testData.questions.length}
      ...
      {/* Question card with feedback on the side */}
